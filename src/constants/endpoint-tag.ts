export const BREAK_LIST_REALTIME_ENDPOINT = "break-list-realtime";
export const BREAK_LIST_ENDPOINT = "breakList";
export const EMPLOYEES_ENDPOINT = "employees";
export const REMARKS_ENDPOINT = "remarks";
export const REMARKS_REALTIME_ENDPOINT = "remarks-realtime";
export const REPORT_BAR_ENDPOINT = "report";
export const REPORT_CUCINA_ENDPOINT = "report-cucina";
export const REPORT_CUCINA_REALTIME_ENDPOINT = "report-cucina-realtime";
export const REPORT_BAR_REALTIME_ENDPOINT = "report-realtime";

export const SEND_TELEGRAM_TTN_ENDPOINT = "send-telegram-ttn";
export const SEND_TELEGRAM_ZN_ENDPOINT = "send-telegram-zn";

export const STOPLIST_ENDPOINT = "stop-list";
export const USER_ENDPOINT = "user";
