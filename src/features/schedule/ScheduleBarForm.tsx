...
import { selectMonthBar } from "./constants";
import { ScheduleTable } from "./ScheduleTable";

export const ScheduleBarForm = () => {
  const t = useTranslations("Navigation");
  const LOKAL_KEY = "lokal-month-bar";
  ...
  const handleChange = (value: string) => {
    const selected = selectMonthBar.find((item) => item.value === value);
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(() => {})} className="w-full">
          <div className="md:w-1/5 w-full pb-3">
            <Select
              value={
                selectMonthBar.find((item) => item.label === dataRange)?.value
              }
